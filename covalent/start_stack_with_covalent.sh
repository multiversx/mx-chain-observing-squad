...
do
   ...
   docker run --rm --mount type=bind,source=${KEYS_FOLDER},destination=/keys --workdir /keys multiversx/multiversx-node-keygenerator:latest && sudo chown $(whoami) ${KEYS_FOLDER}/validatorKey.pem && mv ${KEYS_FOLDER}/validatorKey.pem ${STACK_FOLDER}/node-$OBSERVER/config/observerKey_$OBSERVER.pem
done
