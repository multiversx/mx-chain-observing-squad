...
do
   ...
   docker run --rm --mount type=bind,source=${KEYS_FOLDER},destination=/keys --workdir /keys elrondnetwork/elrond-go-keygenerator:latest && mv ${KEYS_FOLDER}/validatorKey.pem ${STACK_FOLDER}/node-$OBSERVER/config/observerKey_$OBSERVER.pem
done
